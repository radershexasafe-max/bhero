...
class ApiClient {
  ...
  static String friendlyError(
    ...
    if (text.contains('duplicate_product')) {
      return 'This product already exists. Check the name or barcode.';
    }
    if (text.contains('duplicate_supplier')) {
      return 'This supplier already exists.';
    }
    if (text.contains('duplicate_bale_category')) {
      return 'This bale category already exists.';
    }
    if (text.contains('duplicate_bale_label')) {
      return 'This bale label already exists.';
    }
    if (text.contains('duplicate_bale_code')) {
      return 'This bale code already exists.';
    }
    if (text.contains('duplicate_bale_order_item')) {
      return 'This bale order has duplicate lines. Merge repeated category or label entries into one line.';
    }
    if (text.contains('duplicate_bale_order')) {
      return 'That bale order number already exists.';
    }
    if (text.contains('duplicate_bale')) {
      return 'This bale entry already exists.';
    }
    if (text.contains('duplicate_user_email')) {
      return 'A user with this email already exists.';
    }
    if (text.contains('duplicate_username')) {
      return 'That username is already taken.';
    }
    if (text.contains('duplicate_role_profile')) {
      return 'That role name already exists.';
    }
    if (text.contains('duplicate_registration_identity')) {
      return 'A registration with this email or phone number already exists.';
    }
    if (text.contains('already exists') || text.contains('already taken')) {
      return sanitized;
    }
    if (text.contains('duplicate entry') ||
        text.contains('duplicate key') ||
        text.contains('1062') ||
        (text.contains('23000') && text.contains('duplicate')) ||
        text.startsWith('duplicate_')) {
  ...
  static String _preferredBaseUrl(String rawBaseUrl) {
    ...
    if (host.isEmpty) return cleaned;
    if (_forceHttpHosts.contains(host) && parsed.scheme.toLowerCase() != 'http') {
      return parsed.replace(scheme: 'http').toString().replaceFirst(RegExp(r'/+$'), '');
    }
    return cleaned;
  }
  ...
  static Uri _uriForBase(String effectiveBaseUrl, String path, [Map<String, dynamic>? query]) {
    ...
    return Uri.parse('${_preferredBaseUrl(effectiveBaseUrl)}/api$path')
  ...
  static List<String> _candidateBaseUrlsFor(String rawBaseUrl) {
    final preferred = _preferredBaseUrl(rawBaseUrl);
    final candidates = <String>[preferred];
    if (preferred.startsWith('https://')) {
      candidates.add('http://${preferred.substring('https://'.length)}');
    }
  ...
  Future<Map<String, dynamic>> importCustomersFile({
    ...
    Object? lastError;
    for (final candidate in _candidateBaseUrlsFor(baseUrl)) {
      final uri = _uriForBase(candidate, '/customers/import');
      try {
