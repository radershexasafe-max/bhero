      ...
      builder: (ctx) {
        ...
        return StatefulBuilder(
          builder: (ctx, setDialogState) => AlertDialog(
            scrollable: true,
            title: Text(bale == null ? 'Add Bale' : 'Edit Bale'),
            content: SizedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ValueListenableBuilder<int?>(
                    ...
                    builder: (_, value, __) => _searchChoiceField(
                      label: 'Bale category',
                  ...
                  ValueListenableBuilder<int>(
                    ...
                    builder: (_, value, __) => DropdownButtonFormField<int>(
                      value: value,
                      isExpanded: true,
                      decoration: const InputDecoration(labelText: 'Bale grade'),
                      items: const [
                  ...
                  ValueListenableBuilder<String>(
                    ...
                    builder: (_, value, __) => TextField(
                      ...
                      decoration: InputDecoration(
                        labelText: value == 'KGS' ? 'KGS' : 'Pieces',
                      ),
                    ),
                  ),
                  const SizedBox(height: 8),
                  TextField(
                    ...
                    enabled: !saving,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Bale cost price'),
                  ),
                  const SizedBox(height: 8),
                  TextField(
                    controller: sellCtrl,
                    enabled: !saving,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Bale selling price'),
                  ),
                ],
              ),
            ),
                      ...
                      : () async {
                          final confirmed = await showDialog<bool>(
                            ...
                            builder: (confirmCtx) => AlertDialog(
                              title: const Text('Delete Bale'),
                              content: Text('Delete ${(bale['product_name'] ?? 'this bale').toString()}?'),
                              actions: [
                                TextButton(
                ...
                onPressed: saving
                    ...
                    : () async {
                        ...
                        if (categoryId.value == null) {
                          setDialogState(() => dialogError = 'Select a bale category.');
                          return;
                        }
                        if (productId.value == null) {
                          setDialogState(() => dialogError = 'Select a bale product.');
                          return;
                        }
                        if (labelId.value == null) {
                          setDialogState(() => dialogError = 'Select a bale label.');
                          return;
                        }
                        if (qty == null || qty <= 0) {
                          setDialogState(() => dialogError = 'Enter a valid bale quantity.');
                          return;
                        }
                        if (cost == null || cost < 0) {
                          setDialogState(() => dialogError = 'Enter a valid bale cost price.');
                          return;
                        }
                        if (sell == null || sell < 0) {
                          setDialogState(() => dialogError = 'Enter a valid bale selling price.');
                          return;
                        }
                        final confirmed = await showDialog<bool>(
                          context: ctx,
                          builder: (confirmCtx) => AlertDialog(
                            title: Text(bale == null ? 'Save Bale' : 'Update Bale'),
                            content: Text(
                              bale == null
                                  ? 'Save this bale item?'
                  ...
                  children: [
                    ...
                    Align(
                      ...
                      child: Wrap(
                        ...
                        runSpacing: 8,
                        children: [
                          MobileMetricChip('Bales ${_bales.length}'),
                          if (_categories.isNotEmpty)
                            MobileMetricChip('Categories ${_categories.length}'),
                          if (_labels.isNotEmpty)
                            MobileMetricChip('Labels ${_labels.length}'),
                        ],
                      ),
                    ),
                  ],
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${(bale['category_name'] ?? '-')} | ${(bale['label_name'] ?? '-')} | Grade ${bale['grade'] ?? '-'}',
                          style: const TextStyle(
                            color: Colors.black54,
                        ...
                        const SizedBox(height: 6),
                        Text(
                          '${bale['unit_quantity'] ?? 0} ${(bale['unit_of_measure'] ?? '').toString().toLowerCase()}',
                        ...
                        if (((bale['location_stock_summary'] ?? '').toString()).trim().isNotEmpty)
                          Text(
                            'Location stock: ${(bale['location_stock_summary'] ?? '').toString()}',
                            style: const TextStyle(color: Colors.black54),
                          ),
                        if (((bale['location_stock_summary'] ?? '').toString()).trim().isNotEmpty)
                          const SizedBox(height: 8),
                        Text(
                          'Created ${((bale['created_at'] ?? '')).toString()}',
                          style: const TextStyle(color: Colors.black54),
                        ),
                        const SizedBox(height: 12),
                        Wrap(
