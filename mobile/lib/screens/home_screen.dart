...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadHeaderLogo() async {
    try {
      final branding = await PrinterService().getReceiptBranding();
      if (!mounted) return;
      setState(() => _headerLogoPath = branding.logoPath);
    } catch (_) {
      // Keep the default tenant/app logo if branding cannot be loaded.
    }
  }
  ...
  Widget build(BuildContext context) {
    final tenantName = widget.appState.tenant?.name ?? 'T.One';
            ...
            child: Padding(
              ...
              child: BrandLogoImage(
                logoPath: resolveBrandLogoPath(
                  overrideLogoPath: _headerLogoPath,
