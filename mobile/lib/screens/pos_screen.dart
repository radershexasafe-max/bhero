...
class _PosScreenState extends State<PosScreen> {
  ...
  Future<void> _loadCachedCatalog(String term) async {
    final cached = term.isEmpty
        ? await LocalDb.instance.getCachedProducts(limit: 30)
        : await LocalDb.instance.searchCachedProducts(term);
    if (!mounted) return;
    setState(() {
      final mapped = cached.map(_productAsBale).toList();
      _featuredBales = term.isEmpty ? mapped.take(20).toList() : const [];
  ...
  Future<void> _loadCatalog({String? search, bool quiet = false}) async {
    ...
    try {
      if (_online) {
        final featuredRaw = term.isEmpty
            ? await api.getBales(locationId: _locationId, limit: 24)
            : null;
        final featured = _availableBales(featuredRaw ?? _featuredBales);
        final resultRaw = term.isEmpty
            ? (featuredRaw ?? const <Map<String, dynamic>>[])
            : await api.getBales(
                search: term,
                locationId: _locationId,
                limit: 60,
              );
        final results = _availableBales(resultRaw);
        if (!mounted) return;
        setState(() {
          if (featuredRaw != null || _featuredBales.isEmpty) {
            _featuredBales = featured.take(20).toList();
          }
          _results = term.isEmpty ? const [] : results;
          _catalogError = null;
        });
      } else {
    ...
    try {
      ...
      if (_online) {
        ...
        if (list.isNotEmpty) {
          ...
          final products = await api.getProducts(barcode: code);
          if (products.isNotEmpty) {
            bale = _productAsBale(products.first);
          }
        }
      } else {
